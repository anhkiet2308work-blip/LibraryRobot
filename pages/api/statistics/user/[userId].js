// API endpoint cho thống kê người dùng cụ thể
...
export default async function handler(req, res) {
  const { userId, period = 'month' } = req.query; // month hoặc week
  ...
  try {
    ...
    if (period === 'week') {
      // Thống kê theo tuần (4 tuần trong tháng hiện tại)
      const now = new Date();
